package com.huizhixin.smart.modules.shiro.authc;
...
import com.huizhixin.smart.common.util.RedisUtil;
...
import com.huizhixin.smart.modules.system.service.ISysDepartService;
import com.huizhixin.smart.modules.system.service.ISysUserDepartService;
...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		if (token == null) {
			log.info("————————身份认证失败——————————IP地址:  "
					+ oConvertUtils.getIpAddrByRequest(SpringContextUtils.getHttpServletRequest()));
	...
	public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
		...
		List<String> roleCodeList = sysUserService.getRole(username);
		sysUser.setRoleCodeList(roleCodeList);
		if (roleCodeList != null && roleCodeList.size() > 0) {
			sysUser.setRoleCode(roleCodeList.get(0));
		}
		List<SysDepart> departList = sysDepartService.queryUserDeparts(sysUser.getId());

		if (departList != null && departList.size() > 0) {
			sysUser.setDepartId(departList.get(0).getId());
			
			List<String> departListTemp = new ArrayList<String>();
			for (SysDepart sysDepart : departList) {
				departListTemp.add(sysDepart.getId());
			}
			sysUser.setDepartIdList(departListTemp);
		}
	...
	/**
	 * JWTToken刷新生命周期 （解决用户一直在线操作，提供Token失效问题） 1、登录成功后将用户的JWT生成的Token作为k、v存储到cache缓存里面(这时候k、v值一样) 2、当该用户再次请求时，通过JWTFilter层层校验之后会进入到doGetAuthenticationInfo进行身份验证
	 ...
	 * 7、注：当前端接收到Response的Header中的Authorization值会存储起来，作为以后请求token使用 参考方案：https://blog.csdn.net/qq394829044/article/details/82763936
	 * 
	 * @param userName
	 * @param passWord
	 * @return
